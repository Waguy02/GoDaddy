...
class TransformerPredictor(nn.Module):
    ...
    def setup_network(self):
        ...
        if self.use_census:
            self.cfips_encoder = nn.Linear(N_DIMS_COUNTY_ENCODING,self.emb_dim)
    ...
    def forward(self, X):
        ...
        if self.use_census:
            query = self.cfips_encoder(X[:, 0, :N_DIMS_COUNTY_ENCODING])  # Encoding of the cfips. (It is the same for all the sequence)
