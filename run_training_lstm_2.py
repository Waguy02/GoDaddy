...
def main(args):
    model_name = "base_lstm2_seq_6_ae_h2_nh1" if args.model_name is None else args.model_name



    experiment_dir = os.path.join(EXPERIMENTS_DIR, model_name)

    network = LstmPredictor2(experiment_dir=experiment_dir, hidden_dim=2, n_hidden_layers=1, use_encoder=args.use_census).to(DEVICE)
